...
BANDS = [
    ("Sub-bass", 20, 60, "#e53935"),      # Red - Deep sub/kick
    ("Bass", 60, 250, "#fb8c00"),        # Orange - Bass guitar/low snare
    ("Low-Mid", 250, 500, "#fdd835"),    # Yellow - "Body" of the sound
    ("Mid", 500, 2000, "#43a047"),       # Green - Vocals and instruments
    ("High-Mid", 2000, 4000, "#00acc1"), # Cyan - Clarity and "bite"
    ("Presence", 4000, 6000, "#1e88e5"), # Blue - Vocal definition
    ("Air", 6000, 20000, "#8e24aa"),     # Purple - Shimmer and sparkle
]
...
def find_peak_frequency(fft_magnitudes, freq_array, min_freq=50):
    ...
    valid_indices = np.where(freq_array >= min_freq)[0]
    
    if len(valid_indices) == 0:
        return 0.0
    
    peak_idx_within_valid = np.argmax(fft_magnitudes[valid_indices])
    
    actual_peak_idx = valid_indices[peak_idx_within_valid]
    
    return float(freq_array[actual_peak_idx])
